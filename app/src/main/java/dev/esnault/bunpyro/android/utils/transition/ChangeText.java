...
package dev.esnault.bunpyro.android.utils.transition;
...
/**
 * Quick fix of {@link com.transitionseverywhere.ChangeText} for textIsSelectable
 * {@link TextView}s.
 *
 ...
 *
 * The quick fix is needed because when textIsSelectable is set to true, calling
 * {@link TextView#setText(CharSequence)} will actually update the text as a
 * Spannable to add multiple selection based and TextWatcher spans.
 * Because of that, we can't compare directly the text we set with the text the
 * text view has.
 *
 * The quick fix is annotated with //WORKAROUND
 */
            ...
            if (mChangeBehavior == CHANGE_BEHAVIOR_KEEP) {
                ...
                anim.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        // WORKAROUND
                        if (isSameText(startText, view.getText())) {
